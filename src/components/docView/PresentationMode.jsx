...
export function PresentationMode({ 
  ...
  useEffect(() => {
    const handleKeyPress = (e) => {
      switch(e.key) {
        ...
        case 'Escape': handleCloseRequest(); break;
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      exit="exit" // Ensure exit animation runs
      variants={overlayVariants}
      className="fixed inset-0 z-[1000] bg-white flex flex-col touch-none" // Changed to bg-black, increased z-index
      onMouseMove={() => setControlsVisible(true)}
      onMouseLeave={() => setControlsVisible(false)}
      style={{ isolation: 'isolate' }} // Prevent bleed-through
    >
      {/* Main Content Area */}
      <div className="flex-1 relative overflow-hidden bg-white"> {/* Added bg-black */}
            ...
            <AnimatePresence>
              {showCloseConfirm && (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[1001]" // Increased z-index
      ...
      <motion.div
        ...
        className="fixed inset-0 pointer-events-none z-[1001]" // Increased z-index
